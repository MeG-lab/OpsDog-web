...
- 对话式运维助手：通过模型、MCP、任务能力和 Workflow 完成任务。
- 即时任务：一次性执行脚本或工具。
...
flowchart TB
  ...
  subgraph Backend["server/src 后端"]
    ...
    SkillRegistry["skillRegistry.js<br/>旧版意图绑定兼容"]
...
UI 层应尽量只处理展示、交互和轻量状态派发。涉及后端调用、MCP 规划、任务能力意图识别、持久化同步时，应放到 `services/` 或 `stores/`。

### 6.2 状态层：`src/stores`

| 文件 | 职责 |
| --- | --- |
| `stores/index.ts` | 当前主 Zustand store，包含 UI 状态、配置、Server、旧版绑定、对话状态和持久化触发 |
| `stores/appearance.ts` | 主题和背景预设读写 |

当前 `stores/index.ts` 同时承载 AppState、ChatState、持久化快照构建和初始化逻辑。后续如果继续扩展，应优先拆成配置、对话、Server/任务能力、UI 外观等独立 slice。
...
| `webRuntime.ts` | 浏览器环境下的 HTTP API 调用、本地缓存、MCP/任务能力/Server/报告/资产方法 |
| `chatExecutor.ts` | 对话执行编排：任务能力、MCP、Workflow、模型调用 |
| `mcpChatPlanner.ts` | MCP 工具定义、文件系统意图解析、工具结果格式化 |
| `webRouting.ts` | Web 端对话路由和执行计划 |
| `webSkills.ts` | 旧版绑定的前端 fallback 元信息 |
...
| `src/types/index.ts` | UI、对话、Server、任务能力、旧版绑定、资产、运行结果等领域类型 |
...
### 7.5 任务能力、旧版绑定与 Workflow

| 文件 | 职责 |
| --- | --- |
| `skillRegistry.js` | 只读兼容旧版 `tools/skills/*/skill.yaml` 绑定 |
| `workflowRegistry.js` | 基于 workflowId 执行工具链 |
| `tools/script/**/*.server.json` | 任务能力的 Server/Tool 元数据 |
| `tools/skills/*/skill.yaml` | 旧版意图绑定，保留读取兼容，不再新增 |

当前概念关系：

- Server 是可执行能力的底座。
- Tool 是 Server 暴露的具体调用单元。
- Task 是用户上传或平台维护的本地脚本任务，由 `.server.json` 描述。
- Intent 是模型基于 Server/Tool 元数据做出的自然语言到工具选择。
- Skill 只保留给后续可发布、可导入、可分发的外部 Skill 包。
- 旧版 `skill.yaml` 只作为兼容层提供默认参数和历史提示词。
...
| 旧版绑定 | `/api/skills/*` | 旧版绑定列表、更新、删除；新增已停用 |
...
sequenceDiagram
  ...
  alt 选择任务能力或 Workflow
    Runtime->>API: /api/servers/:id/tools/:toolName/call 或 /api/workflows/execute
    API->>MCP: 执行 Server Tool 或流程
  else 命中 MCP
...
### 10.3 Intent 到 Server/Tool 的执行流

```mermaid
flowchart LR
  UserText["用户自然语言"]
  Planner["模型意图规划<br/>Server/Tool metadata"]
  ServerDef["ServerDefinition<br/>instant / managed / system"]
  Tool["ServerToolDefinition"]
  Runner["pythonServerRunner<br/>或 MCP call"]
  Result["ExecutionResult"]
  Legacy["tools/skills/*/skill.yaml<br/>只读兼容提示"]

  UserText --> Planner
  Legacy -.兼容合并.-> ServerDef
  Planner --> ServerDef
  ServerDef --> Tool
  Tool --> Runner
  Runner --> Result
...
`App.tsx` 每 3 秒刷新 Server 状态，比较前后状态。当托管任务从正常进入告警，或从告警恢复时，会写入系统公告；如果配置了语音服务，并满足冷却条件，会调用语音通知任务能力。
...
3. 确认 `serverRegistry.js` 能扫描到 Server。
4. 在 UI 中验证任务列表和对话意图识别。
5. 更新相关文档：结构文档、使用说明或开发说明。
...
4. 验证连接、工具列表和调用结果。
5. 如需业务化触发，优先完善 Tool 描述、inputSchema 或 Workflow。
...
- `Skill` 不再指脚本触发词绑定；本轮以后只保留给后续外部 Skill 包。
- `tools/skills/*/skill.yaml` 是旧版意图绑定，只读兼容，不应新增。
